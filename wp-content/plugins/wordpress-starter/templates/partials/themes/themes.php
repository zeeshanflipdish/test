<div class="sg-grid sg-grid--gap-responsive sg-grid--autoflow-row sg-tabs-<?php echo $tab['id']; ?> sg-tab-<?php echo $tab['id']; ?> sg-tabs_content <?php echo ( ! empty( $tab['active'] ) ? 'sg-tabs_content--active' : '' ) ?>">
	<div class="filter-drawer">
		<div class="buttons">
			<button type="button" class="apply-filters button"><?php _e( 'Apply Filters', 'siteground-wizard' ); ?><span></span></button>
			<button type="button" class="clear-filters button" aria-label="<?php esc_attr_e( 'Clear current filters', 'siteground-wizard' ); ?>"><?php _e( 'Clear', 'siteground-wizard' ); ?></button>
		</div>
	</div>
